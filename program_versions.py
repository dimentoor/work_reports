...
class ProgramVersions:
    ...
    def __init__(self, path, sheet_name):
        ...
        self.program_versions_text = "На листе program_versions_sample представлены программы и их количество, " \
                                     "для каждой из которых указаны номера версий."

        self.alive = 0
        self.alive_text = "На листе alive_sample представлены имена устройств пользователей, группы, в которых " \
                          "находятся эти устройства и дата последнего появления устройства в сети."

        self.filtered_df = 0
        self.filtered_df_text = "На листе filtered_alive представлены имена устройств пользователей, группы, " \
                                "в которых находятся эти устройства и дата последнего появления устройства в сети. В " \
                                "выборку отбираются только устройства, дата последнего появления в сети которых " \
                                "отличающиеся от даты произведения анализа отчета на 7 или более дней."
        # self.updates = 0
        self.unique = 0
        self.unique_text = "Отчет о версиях программ \n\n На листе unique_sample представлено количество уникальных " \
